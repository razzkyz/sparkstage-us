-- Read-only voucher validation for frontend (does not reserve quota)
-- Supports parent categories (e.g. Aksesoris -> Bangle)
...
BEGIN
  IF auth.role() NOT IN ('authenticated', 'service_role') THEN
    RETURN QUERY SELECT NULL::UUID, NULL::TEXT, NULL::NUMERIC, NULL::NUMERIC, 'Unauthorized'::TEXT;
    RETURN;
  END IF;
  ...
  IF v_voucher.min_purchase IS NOT NULL AND p_subtotal < v_voucher.min_purchase THEN
    RETURN QUERY SELECT NULL::UUID, NULL::TEXT, NULL::NUMERIC, NULL::NUMERIC, FORMAT('Minimum pembelian Rp %s', v_voucher.min_purchase)::TEXT;
    RETURN;
  END IF;
