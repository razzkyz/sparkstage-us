-- ============================================================
-- Migration: Fix rental_orders source column in RPCs
-- Date: 2026-06-03
-- Safe: CREATE OR REPLACE only
-- ============================================================

-- Update create_rental_order_admin (admin dashboard order creation)
...
BEGIN
  ...
  IF v_caller_id IS NULL THEN
    RETURN jsonb_build_object('error', 'Login diperlukan');
  END IF;

  IF p_customer_name IS NULL OR trim(p_customer_name) = '' THEN
    RETURN jsonb_build_object('error', 'Nama lengkap wajib diisi');
  END IF;
  IF p_customer_phone IS NULL OR trim(p_customer_phone) = '' THEN
    RETURN jsonb_build_object('error', 'Nomor HP wajib diisi');
  END IF;
  IF p_duration_days < 1 THEN
    RETURN jsonb_build_object('error', 'Durasi sewa minimal 1 hari');
  END IF;
  ...
  v_order_number := 'RTL-' || to_char(NOW() AT TIME ZONE 'Asia/Jakarta', 'YYYYMMDD') || '-' ||
  ...
  LOOP
    ...
    v_daily_rate   := COALESCE((v_item->>'daily_rate')::BIGINT, 35000);
    v_item_deposit := COALESCE((v_item->>'deposit_amount')::BIGINT, 50000);

    IF v_variant_id IS NULL THEN
      RETURN jsonb_build_object('error', 'Item tidak valid');
    END IF;
    ...
    IF NOT FOUND THEN
      RETURN jsonb_build_object('error', 'Produk tidak ditemukan atau sudah tidak tersedia');
    END IF;
  ...
  INSERT INTO public.rental_orders (
    ...
    customer_address, start_time, end_time, duration_days,
    total_rental_cost, total_deposit, total_amount,
    status, payment_status, source, notes, created_at, updated_at
  ) VALUES (
    v_order_number, v_caller_id, p_customer_name,
    COALESCE(p_customer_email, ''), p_customer_phone,
    COALESCE(p_customer_address, 'Ambil di tempat'),
    p_rental_start_time, v_rental_end_time, p_duration_days,
    v_total_rental, v_total_deposit, v_total_amount,
    'pending', 'unpaid', 'formal', p_notes, NOW(), NOW()
  )
  ...
  LOOP
    ...
    INSERT INTO public.rental_order_items (
      rental_order_id, dressing_room_product_variant_id, product_name,
      quantity, daily_rate, item_deposit_amount, total_rental_cost,
      current_status, status_updated_at, created_at, updated_at
    ) VALUES (
      v_order_id, v_variant_id, v_product_name || ' – ' || v_variant_label,
      v_quantity, v_daily_rate, v_item_deposit, v_item_rental_cost,
      'reserved', NOW(), NOW(), NOW()
    )
    ...
    INSERT INTO public.rental_item_status_history (
      rental_order_id, rental_order_item_id, status, previous_status, reason, created_by
    ) VALUES (
      v_order_id, v_item_order_id, 'reserved', NULL,
      'Order dibuat oleh admin', v_caller_id
    );
  END LOOP;
...
-- Update create_rental_order_public (customer storefront order creation)
CREATE OR REPLACE FUNCTION public.create_rental_order_public(
  p_customer_name     TEXT,
  p_customer_email    TEXT,
  p_customer_phone    TEXT,
  p_rental_start_time TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
BEGIN
  v_caller_id := auth.uid();
  IF v_caller_id IS NULL THEN
    RETURN jsonb_build_object('error', 'Login diperlukan untuk membuat pemesanan sewa');
  END IF;
  ...
  -- Add days to the start time (automatically handles accurate 24-hours addition)
  v_rental_end_time := p_rental_start_time + (p_duration_days || ' days')::INTERVAL;

  -- Generate unique order number with RTL prefix (using Jakarta timezone for the YYYYMMDD date format)
  v_order_number := 'RTL-' || to_char(NOW() AT TIME ZONE 'Asia/Jakarta', 'YYYYMMDD') || '-' ||
                    upper(substr(md5(random()::TEXT || clock_timestamp()::TEXT), 1, 6));
  ...
  LOOP
    ...
    INSERT INTO public.rental_item_status_history (
      ...
      v_order_id, v_item_order_id, 'reserved', NULL,
      'Order dibuat customer via storefront - menunggu konfirmasi', v_caller_id
    );
  END LOOP;
...
GRANT EXECUTE ON FUNCTION public.create_rental_order_public(
  TEXT, TEXT, TEXT,
