-- ============================================
-- Migration: Create Rental Order Public RPC
-- Date: 2026-05-27
-- Description: Atomic RPC for authenticated customers to create a
--   dressing room rental order from the storefront (pickup only).
-- ============================================

-- Ensure notes and source columns exist (safe to run multiple times)
ALTER TABLE public.rental_orders ADD COLUMN IF NOT EXISTS notes TEXT;
ALTER TABLE public.rental_orders ADD COLUMN IF NOT EXISTS source TEXT DEFAULT 'online';
...
BEGIN
  ...
  -- Insert rental_order (using the new schema with rental_start_time etc.)
  ...
  LOOP
    ...
    INSERT INTO public.rental_item_status_history (
      ...
      'reserved',
      NULL,
      'Order dibuat customer via storefront - menunggu konfirmasi',
      v_caller_id
    );
  END LOOP;
