-- Allow QR scanning 30 minutes before session start time
-- Session 1: Entry at 9:00, allow scan from 8:30
-- Session 2: Entry at 12:00, allow scan from 11:30
-- Session 3: Entry at 15:00, allow scan from 14:30
...
BEGIN
  ...
  IF v_ticket.time_slot IS NOT NULL THEN
    ...
    v_scan_start := v_session_start - INTERVAL '30 minutes';

    IF v_now < v_scan_start THEN
      RETURN jsonb_build_object(
        'ok', false,
        'code', 'session_not_started',
        'message', format(
          'Sesi belum dimulai. Scan dibuka jam %s, masuk jam %s.',
          to_char(v_scan_start AT TIME ZONE 'Asia/Jakarta', 'HH24:MI'),
