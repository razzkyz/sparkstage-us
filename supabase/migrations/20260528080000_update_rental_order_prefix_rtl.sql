-- ============================================================
-- Migration: Update rental order number prefix from RNT to RTL
-- Date: 2026-05-28
-- Safe: CREATE OR REPLACE only — no data deleted
-- ============================================================

-- Update generate_rental_order_number (used by triggers / manual calls)
...
BEGIN
  ...
  INSERT INTO public.rental_orders (
    ...
    customer_address, rental_start_time, rental_end_time, duration_days,
    ...
    'pending', 'unpaid', 'admin', p_notes, NOW(), NOW()
  )
...
BEGIN
  ...
  -- Generate unique order number with RTL prefix
  v_order_number := 'RTL-' || to_char(NOW(), 'YYYYMMDD') || '-' ||
                    upper(substr(md5(random()::TEXT || clock_timestamp()::TEXT), 1, 6));
  ...
  INSERT INTO public.rental_orders (
    ...
    p_rental_start_time, v_rental_end_time, p_duration_days,
    v_total_rental, v_total_deposit, v_total_amount,
    'pending', 'unpaid', 'online', p_notes, NOW(), NOW()
  )
  RETURNING id INTO v_order_id;
