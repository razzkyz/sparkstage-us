-- Fix trigger: guard against non-time values like 'all-day' in time_slot
-- When time_slot is 'all-day' or unparseable, treat as NULL (skip capacity update for that slot)
...
DECLARE
  ...
  v_new_slot TEXT;
BEGIN
  -- Safely normalize time slots to HH:MM:SS text
  -- Guard: skip 'all-day', NULL, or unparseable values
  v_old_slot := CASE
