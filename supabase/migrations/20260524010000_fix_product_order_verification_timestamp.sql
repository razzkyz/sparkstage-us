-- Fix product order verification timestamp accuracy
-- Add verified_at column to track exact verification time separately from updated_at

-- PHASE 1: Add verified_at column to order_products
ALTER TABLE public.order_products
  ADD COLUMN IF NOT EXISTS verified_at TIMESTAMPTZ;

-- PHASE 2: Backfill verified_at for completed orders
-- Use updated_at as the verified time for already completed orders
UPDATE public.order_products
SET verified_at = updated_at
WHERE pickup_status = 'completed' AND verified_at IS NULL AND updated_at IS NOT NULL;

-- PHASE 3: Update complete_product_pickup_atomic RPC to set verified_at
...
-- PHASE 4: Update complete_cashier_product_pickup_atomic RPC
CREATE OR REPLACE FUNCTION public.complete_cashier_product_pickup_atomic(
  p_pickup_code TEXT,
  p_picked_up_by UUID
)
RETURNS JSONB
...
-- PHASE 5: Update frontend helper to use verified_at
-- This is a documentation note - update productOrdersHelpers.ts to use verified_at instead of updated_at
-- Change: formatDateTimeWIB(order.updated_at) to formatDateTimeWIB(order.verified_at)
