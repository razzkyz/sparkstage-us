...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[cancel-ticket-order] start', trace)

    const supabase = createServiceClient(auth.supabaseEnv.url, auth.supabaseEnv.serviceRoleKey)
    const { data, error } = await supabase.rpc('cancel_ticket_order_atomic', {
      p_order_number: orderNumber,
      p_user_id: auth.user.id,
    })

    if (error) {
      console.error('[cancel-ticket-order] rpc_error', {
        ...trace,
        error: error.message,
      })
    ...
    const result =
      data && typeof data === 'object' ? (data as CancelTicketOrderResult) : ({ ok: false } satisfies CancelTicketOrderResult)

    console.log('[cancel-ticket-order] end', {
      ...trace,
    ...
    console.error('[cancel-ticket-order] Unhandled error:', errorMessage)
    return jsonErrorWithDetails(req, 500, {
