    ...
    <PageTransition>
      <div className="min-h-screen bg-gradient-to-b from-white via-rose-50/10 to-white">
        {settings.section_order.map((sectionId, index) => {
          ...
          const bgClass = isEven ? 'bg-white' : 'bg-rose-50/5';
                ...
                <div className="max-w-7xl mx-auto">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    <div className={isEven ? '' : 'order-1 lg:order-2'}>
                      <span className="inline-block px-4 py-2 border-2 border-rose-300 text-rose-600 text-xs font-bold uppercase tracking-widest mb-6 rounded-lg hover:bg-rose-50 hover:border-rose-400 transition-all duration-200">
                        {settings.section_1_category}
                      </span>
                      <h1
                        className="text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-gray-900 mb-6 leading-tight border-b-4 border-rose-200 inline-block pb-2"
                    ...
                    {settings.section_1_image && (
                      <div className={`bg-white rounded-xl shadow-sm border-l-4 border-rose-200 overflow-hidden hover:border-rose-400 hover:shadow-lg transition-all duration-300 ${isEven ? '' : 'order-2 lg:order-1'}`}>
                        <img
              ...
              <section key={sectionId} className={`py-20 px-6 md:px-12 lg:px-20 ${bgClass}`}>
                <div className="max-w-7xl mx-auto">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div className={isEven ? 'order-1 lg:order-2' : ''}>
                      <h2
                        className="text-4xl md:text-5xl font-serif font-bold text-gray-900 mb-6 leading-tight whitespace-pre-line"
                      ...
                      <blockquote className="border-l-4 border-rose-400 pl-6 py-4 bg-rose-50/30 rounded-r-lg">
          ...
          if (sectionId === 'section_3') {
            return (
              <section key={sectionId} className={`py-20 px-6 md:px-12 lg:px-20 ${bgClass}`}>
                <div className="max-w-7xl mx-auto">
                  <h2
                    className="text-3xl md:text-4xl font-serif font-bold text-gray-900 mb-12 text-center"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {settings.section_3_products.map((product, pIndex) => (
                      <div key={pIndex} className="bg-white rounded-xl shadow-sm border-l-4 border-rose-200 p-6 hover:border-rose-400 hover:shadow-lg transition-all duration-300">
                        {product.image && (
                        ...
                        <p className="text-xs font-bold text-rose-600 uppercase tracking-widest mb-2">{product.brand}</p>
                        <h3 className="text-lg font-bold text-gray-900 mb-2">{product.name}</h3>
                        <p className="text-gray-600 mb-4">{product.price}</p>
                        {product.link && (
                          <a href={product.link} target="_blank" rel="noopener noreferrer" className="inline-block px-6 py-3 border-2 border-rose-300 text-rose-600 font-semibold rounded-lg hover:bg-rose-50 hover:border-rose-400 transition-all duration-200">
                            Shop Now
                          </a>
                        )}
                      </div>
